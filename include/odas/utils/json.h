...
/*
    unsigned int json_tokens_count(const json_tokens * obj, char * ptr);

    char * json_tokens_gotoEnd(const json_tokens * obj, char * ptr);

    char * json_tokens_getFromObject(const json_tokens * obj, char * parent, char * key);

    char * json_tokens_getFromArray(const json_tokens * obj, char * parent, unsigned int index);

    char * json_tokens_getFromValue(const json_tokens * obj, char * parent);

    void json_tokens_getAllFromArray(const json_tokens * obj, char * parent, char ** dest);


    void json_tokens_extractString(const json_tokens * obj, char * ptr, char * dest);

    void json_tokens_extractFloat(const json_tokens * obj, char * ptr, float * dest);


    char * json_fsm_micro_digit09(char * expr);

    char * json_fsm_micro_digit19(char * expr);

    char * json_fsm_micro_unicode(char * expr);

    char * json_fsm_micro_4hex(char * expr);

    char * json_fsm_micro_space(char * expr);

    char * json_fsm_micro_char(char * expr, const char special);

    char * json_fsm_micro_true(char * expr);

    char * json_fsm_micro_false(char * expr);

    char * json_fsm_micro_null(char * expr);


    char * json_fsm_object(char * expr, json_tokens * tokens, char * parent);

    char * json_fsm_array(char * expr, json_tokens * tokens, char * parent);

    char * json_fsm_value(char * expr, json_tokens * tokens, char * parent);

    char * json_fsm_string(char * expr, json_tokens * tokens, char * parent);

    char * json_fsm_number(char * expr, json_tokens * tokens, char * parent);
*/
